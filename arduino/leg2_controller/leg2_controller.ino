...
// TESTING FUNCTIONS for recalling values that were sent

// copy the current instruction
void getInstruction(int inst) {
  while (Wire.available()) {
    unsigned char byte = Wire.read();
    valueReceived = byte;
  }
...
void recallInstruction() {
  Wire.write(valueReceived);
}
...
void setup()
{
  ...
  Wire.onRequest(recallInstruction);
